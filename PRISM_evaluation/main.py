...
def get_args():
    ...
    parser.add_argument('--size', type=str, required=True, help="large/small")
...
def model_specific_postprocesser(args, prompt, answer):
    if prompt in answer:
        answer = answer.split(prompt, 1)[-1].strip()
        if args.model == "Gemma":
            answer = answer.split("model\n", 1)[-1].strip()
        elif args.model =="LLaVA" or args.model =="LLaVA-Next" or args.model == "Safe-LLaVA-7B":
        ...
        if args.model =="Qwen25" :
            answer = answer[0]
        elif args.model == "Safe-LLaVA-0_5B" or args.model == "LLaVA-0_5B"  or args.model == "Safe-LLaVA-7B" or  args.model == 'LLaVA-7B-baseline':
...
elif args.model =="Safe-LLaVA-7B":
    model = "./checkpoints/Safe-LLaVA-7B"

elif args.model == "LLaVA-0_5B":
    model = "./checkpoints/LLaVA-0_5B"
elif args.model == "LLaVA-7B-baseline":
    model = "./checkpoints/LLaVA-7B-baseline"
...
elif args.model =="Safe-LLaVA-0_5B" or args.model == "LLaVA-Onevision-0_5B" or args.model == "LLaVA-0_5B" or args.model == 'Safe-LLaVA-7B' or args.model == 'LLaVA-7B-baseline':
...
input_path = f"tasks/{args.size}_PRISM_{args.task}.jsonl"
image_base_dir = f"images/{args.size}_images"
output_path = output_path = f"./answer/{args.size}_prism_eval/{args.model}/{args.task}_Answer.jsonl"
...
with open(input_path, "r") as f:
    ...
    for line in tqdm(lines, desc=f"Evaluating {args.model} on {args.task}"):
        ...
        image_path = os.path.join(image_base_dir, item["image"].split('/')[-1])
        prompt = item["text"]
        ...
        if args.model=="LLaVA" or args.model=="LLaVA-Next" or args.model =="LLaVA-Onevision" :
            ...
            output = llm.generate(**inputs, max_new_tokens=200, do_sample=False) #200
            ...
            generated_ids = llm.generate(**inputs, max_new_tokens=100)
            ...
            if '<|' in answer or answer == prompt:
                while '<|' in answer and patience<=10:
        ...
        answer = model_specific_postprocesser(args, prompt, answer)
